...
DEFAULT_MODEL_TAG = "esm2_650m"
DEFAULT_EMBEDDING_DIR = Path(os.environ.get("PLM_BASE_DIR", SCRIPT_DIR / "embeddings"))
...
def infer_input_dim(representation: str, max_length: int, embed_dim: int, reduce_dim: Optional[int]) -> int:
    ...
    if representation == "P":
        if reduce_dim is None:
            raise ValueError("P requires reduce_dim.")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run ESM2-650M F/S/P representation experiments with an MLP.")
    ...
    parser.add_argument("--models", default="mlp", help="Only mlp is supported in this cleaned ESM2 script.")
...
def default_h5_candidates(embedding_dir: Path, model_tag: str, split: str) -> List[Path]:
    legacy_split = "validation" if split == "validation" else split
    return [
        embedding_dir / f"esm2-500t-{legacy_split}_streamv3_maxlen500.h5",
        embedding_dir / f"esm2-500t-{legacy_split}v2-pad500.h5",
        embedding_dir / f"res-canji-500-pad-{legacy_split}.h5",
        embedding_dir / f"{model_tag}_{legacy_split}_maxlen500.h5",
        embedding_dir / f"esm2_650m_{legacy_split}_maxlen500.h5",
    ]
...
def resolve_default_h5_paths(args: argparse.Namespace) -> None:
    ...
    if missing:
        ...
        raise FileNotFoundError(
            "Cannot find ESM2-650M embedding H5 files. Checked:\n"
            f"{checked}\n"
...
def main() -> None:
    ...
    if unsupported_models:
        raise ValueError(f"Only mlp is supported in this ESM2 script, got: {unsupported_models}")
    representations = [r.upper() for r in parse_csv_list(args.representations)]
    unsupported_representations = sorted(set(representations) - {"F", "S", "P"})
    if unsupported_representations:
        raise ValueError(
            "This script handles F/S/P only; use 03_train_n_representations.py "
            f"for N. Got: {unsupported_representations}"
        )
    dims = parse_csv_list(args.dims, int)
    print("ESM2-650M representation sweep paths:")
    ...
    for representation in representations:
        dim_list: List[Optional[int]] = dims if representation == "P" else [None]
    ...
    summary = {
        ...
        "results_csv": str(output_dir / "representation_sweep_results.csv"),
    }
    (output_dir / "representation_sweep_summary.json").write_text(json.dumps(summary, indent=2), encoding="utf-8")
